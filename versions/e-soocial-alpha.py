...
def atualizar_header(chave, valor):
    ...
    # Substituindo o valor na linha correspondente à chave
    for i, linha in enumerate(linhas):
        if f'"{chave}":' in linha:
            linhas[i] = f'"{chave}": "{valor}",\n'
            break

    # Escrevendo as alterações de volta no arquivo
    with open("headers.txt", "w") as file:
        file.writelines(linhas)

# Função chamada ao clicar no botão para inserir o referer
def inserir_referer():
    referer_window = ctk.CTkToplevel(root)
    referer_window.title("Entrada de Referer")
    referer_window.geometry("300x150")  # Ajusta o tamanho da janela
...
def inserir_cookie():
    # Criar uma janela personalizada para obter a entrada
    cookie_window = ctk.CTkToplevel(root)
    cookie_window.title("Entrada de Cookie")
    cookie_window.geometry("300x150")  # Ajusta o tamanho da janela
...
def abrir_painel():
    global root, text_console, text_headers, entry_periodo_apuracao_xml, entry_periodo_apuracao_pdf, label_feedback

    # Cria a janela principal
    root = ctk.CTk()
    root.title("e-Social | V:1.0 ")
    window_width = 800
    window_height = 600
    ...
    #Créditos
    notebook.add("Créditos")
    frame_xml = notebook.tab("Créditos")
